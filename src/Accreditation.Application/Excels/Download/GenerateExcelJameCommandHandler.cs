...
{
    internal sealed class GenerateExcelJameCommandHandler
        : IRequestHandler<GenerateExcelJameCommand, Result<(byte[] FileContent, string FileName)>>
    {

        private readonly IExcelRepository _excelRepository;
        public GenerateExcelJameCommandHandler(IExcelRepository excelRepository)
        {
        ...
        public async Task<Result<(byte[] FileContent, string FileName)>> Handle(GenerateExcelJameCommand request, CancellationToken cancellationToken)
        {
            try
            {
                // Set the file name and format it with a timestamp
                string titleCode = "Modiri";
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    using (var package = new ExcelPackage(memoryStream))
                    {
                        ...
                        worksheet.Cells["A4:F4"].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;
                        worksheet.Cells["A4:F4"].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                        worksheet.Cells["A4:F4"].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(124, 152, 218));
                        ...
                        worksheet.Cells["N4"].Value = "PKنظر ارزیاب";
                        ...
                        // Apply Data Validation for the dropdown in column F
                        // Assuming request.body.NaLists has a boolean true/false for each Sanjeh
                        for (int row = 5; row < 5 + request.Body.NaLists.Count; row++)
                        {
                            var cellAddress = $"F{row}";
